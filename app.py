...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander("❓ 지방은 왜 먹어야 하나요?"):
            st.caption("""
            지방은 **호르몬 생성**의 필수 원료입니다. 
            * **20%:** 운동 에너지가 많이 필요한 경우
            * **30%:** 포만감과 호르몬 건강을 중시하는 경우
            """)

    # 2. 활동량 입력 (0.5 = 30분 가이드 포함)
    st.subheader("⏰ 오늘 나의 24시간 활동량")
    st.info("💡 **입력 가이드:** 슬라이더의 **0.5단위는 30분**을 의미합니다. (예: 1.5 = 1시간 30분)")
    
    col1, col2 = st.columns(2)
    with col1:
        sleep = st.slider("💤 수면 (시간)", 0.0, 12.0, 7.0, 0.5)
        office = st.slider("🪑 좌식 생활 (사무/운전/공부)", 0.0, 15.0, 8.0, 0.5)
        walking = st.slider("🚶 가벼운 활동 (출퇴근/이동)", 0.0, 15.0, 4.0, 0.5)
    with col2:
        labor = st.slider("🏗️ 고강도 활동 (현장/물류/진열)", 0.0, 15.0, 0.0, 0.5)
        workout = st.slider("🏋️ 웨이트 트레이닝 (운동)", 0.0, 4.0, 1.0, 0.5)
        etc = st.slider("☕ 기타 (식사/휴식 등)", 0.0, 10.0, 4.0, 0.5)

    total_h = sleep + office + walking + labor + workout + etc

    if total_h != 24:
        st.warning(f"⚠️ 현재 **{total_h}시간**입니다. 총 24시간을 맞춰주세요! (0.5 = 30분)")
    else:
        # --- 계산 로직 ---
        bmr = 370 + (21.6 * muscle)
        ...
        # 강조된 경고 문구 추가
        st.error("⚠️ **주의사항**\n\n본 계산 결과는 영양학적 공식에 따른 **평균값**입니다. 개인의 활동 강도, 소화 흡수율, 체질에 따라 실제 필요량은 차이가 발생할 수 있습니다. 반드시 **2주 단위**로 본인의 체중 변화를 모니터링하며 칼로리를 미세하게 조정하시기 바랍니다.")
